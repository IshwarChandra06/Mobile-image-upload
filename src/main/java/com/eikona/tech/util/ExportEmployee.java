...
public class ExportEmployee {
	...
	public void fileExportBySearchValue(HttpServletResponse response, String name, String empId,
			 String flag) throws ParseException, IOException {
		
		List<Employee> employeeList = getListOfEmployee( name, empId);
		
		excelGenerator(response, employeeList);
		
	}

	private List<Employee> getListOfEmployee(String name, String empId) {
    	...
    	List<Employee> employeeList =employeeRepository.findAll((employeeNameSpec)
				.and(employeeIdSpec));
		return employeeList;
	}
